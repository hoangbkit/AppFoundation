...
- Added `ProPlansComparisonView` for a dedicated Free-versus-Pro comparison flow from Settings.
- Added Startup Resilience and Developer Tools documentation, portable policy and purchase-simulation tests, and deterministic Demo scenarios.

### Changed

- Made `ProPaywallView` the canonical paywall and deprecated `PaywallView` and `FoundationPaywallView` without removing them, preserving source compatibility for existing clients.
- Flattened `ProPaywallView` into the surrounding themed surface and moved purchase restoration into its legal footer with inline progress and result feedback.
- Redesigned `ProPlanSettingsSection` with a two-line plan identity and horizontally scrollable actions for restore, upgrade, plan comparison, subscription management, and code redemption.
- Extended `ProPlanSettingsConfiguration` with defaulted plan-comparison and code-redemption labels and URLs while retaining its existing initializer arguments.
- Kept Debug simulated-catalog edits separate from the original live `PurchaseConfiguration`, so switching back to Live StoreKit restores the production product identifiers and entitlement policy.
- Simplified the Demo around a single reusable Developer Tools entry and moved user-facing AI provider access to Home.

### Compatibility

- Startup resilience is opt-in and does not change existing app startup behavior.
- Developer Tools and purchase-simulation mutation APIs remain Debug-only and do not alter Release purchase behavior.
- Existing `PaywallView` and `FoundationPaywallView` integrations continue to compile with deprecation warnings; new integrations should use `ProPaywallView`.
- `ProPlanSettingsSection` remains source-compatible, but its refreshed identity replaces the old labeled presentation and derives active plan copy from the StoreKit plan duration.

## 1.3.1 - 2026-08-29

### Changed

- Simplified `ProPaywallView` chrome and kept its navigation title presentation compact.
- Included the active billing duration in `ProPlanSettingsSection` plan labels.
- Hid the Settings restore row while Pro is already active.
