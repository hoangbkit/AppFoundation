...
enum AppAIDirectHTTP {
    ...
    static func validatedMessages(_ values: [AppAIMessage]) throws -> [AppAIMessage] {
        let messages = values.compactMap { message -> AppAIMessage? in
            let content = message.content.trimmingCharacters(in: .whitespacesAndNewlines)
            return content.isEmpty ? nil : AppAIMessage(role: message.role, content: content)
        }
        guard !messages.isEmpty else { throw AppAIDirectError.invalidRequest("At least one non-empty message is required.") }
        return messages
    }
    ...
    static func decode<Value: Decodable>(_ data: Data) throws -> Value {
        ...
        catch { throw AppAIDirectError.invalidResponse }
    }
    ...
    static func response(
        ...
        guard !normalized.isEmpty else { throw AppAIDirectError.emptyOutput }
    ...
    static func error(data: Data, response: HTTPURLResponse) -> AppAIDirectError {
        let message = providerMessage(data: data) ?? HTTPURLResponse.localizedString(forStatusCode: response.statusCode)
        let lowered = message.lowercased()
        switch response.statusCode {
        case 401: return .authenticationFailed
        case 403: return .permissionDenied
        case 402: return .insufficientCredits
        case 404 where lowered.contains("model"): return .modelUnavailable
        case 429:
            if lowered.contains("credit") || lowered.contains("quota") || lowered.contains("billing") {
                return .insufficientCredits
            }
            return .rateLimited(retryAfter: response.value(forHTTPHeaderField: "Retry-After"))
        default:
            if lowered.contains("model") && (lowered.contains("not found") || lowered.contains("unavailable")) {
                return .modelUnavailable
            }
            return .provider(status: response.statusCode, message: message)
        }
    }
...
struct OpenAIResponsesRequest: Encodable {
    struct Input: Encodable {
        struct Content: Encodable { let type = "input_text"; let text: String }
        let role: String
        let content: [Content]
    ...
    struct Text: Encodable {
        ...
        init?(format: AppAIResponseFormat) {
            ...
            case .jsonObject: self.format = .init(type: "json_object", name: nil, schema: nil, strict: nil)
            case .jsonSchema(let schema): self.format = .init(type: "json_schema", name: schema.name, schema: schema.schema, strict: schema.strict)
            }
        }
    }
    let model: String
    ...
    enum CodingKeys: String, CodingKey {
        case model, input, temperature, store, text
...
struct OpenAIResponsesResponse: Decodable {
    ...
    struct Usage: Decodable {
        struct Details: Decodable { let cachedTokens: Int?; enum CodingKeys: String, CodingKey { case cachedTokens = "cached_tokens" } }
        let inputTokens: Int?
...
struct AnthropicResponse: Decodable {
    ...
    let usage: Usage?
    enum CodingKeys: String, CodingKey { case model, content, usage; case stopReason = "stop_reason" }
}
...
struct GeminiRequest: Encodable {
    ...
    struct GenerationConfig: Encodable {
        ...
        let responseSchema: AppAIJSONValue?
    }
...
struct OpenAICompatibleRequest: Encodable {
    ...
    struct ResponseFormat: Encodable {
        ...
        init?(_ format: AppAIResponseFormat) {
            switch format {
            case .text: return nil
            case .jsonObject: type = "json_object"; jsonSchema = nil
            case .jsonSchema(let schema):
                type = "json_schema"
                jsonSchema = .init(name: schema.name, schema: schema.schema, strict: schema.strict)
            }
        }
    }
    let model: String
...
struct OpenAICompatibleResponse: Decodable {
    struct Choice: Decodable {
        struct Message: Decodable { let content: FlexibleText }
        let message: Message
        let finishReason: String?
