#if os(macOS) && canImport(AVFoundation) && canImport(SwiftUI)
  ...
  public final class PromoVideoExporter {
    ...
    public func export(
      ...
      let destination = outputURL ?? makeOutputURL(projectName: project.name)
      ...
      guard writer.startWriting() else {
        throw PromoVideoExportError.exportFailed(
          writer.error?.localizedDescription
            ?? PromoVideoExportError.cannotStartWriter.localizedDescription
        )
      }
      ...
      do {
        for frameIndex in 0..<frameCount {
          ...
          if frameIndex.isMultiple(of: max(frameRate.rawValue / 3, 1))
