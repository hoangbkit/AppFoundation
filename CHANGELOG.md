...
- Added `FoundationVisualStyle` with Signature, Native, Flat, and Glass presets.
- Added root-level `.appFoundationStyle(_:)` environment installation.
- Added full style adoption for `ProPaywallView`, `ProUpsellView`, `ProCelebrationView`, `ProPlanSettingsSection`, `ThemePickerView`, `FoundationOnboardingView`, and `FoundationSettingsView`.
- Added a persisted visual-style picker to the Demo app.

### Changed

- Generalized alternate icon switching while preserving `ThemeAppIconManager` and `ThemeAppIconError` source compatibility.
- Made shared backgrounds, cards, pills, primary actions, Pro artwork, navigation chrome, and Widget Showcase bridging style-aware.

### Documentation

- Added app-icon picker registration and Settings adoption guidance.
- Added built-in visual-style integration guidance.
