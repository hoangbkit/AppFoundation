...
Install a presentation style beside the active app theme:
...
Available presets are `.signature`, `.native`, `.flat`, and `.glass`. The default `.signature` style preserves existing built-in view presentation. See [Built-in View Visual Styles](Documentation/VisualStyles.md) for adopted components, customization, and Demo usage.
