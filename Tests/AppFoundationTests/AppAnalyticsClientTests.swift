...
@Test func analyticsFlushMatchesNativeServerContract() async throws {
    ...
    #expect(requestID.hasPrefix("native-"))
    #expect(request.value(forHTTPHeaderField: "X-Request-ID") == requestID)
    ...
    #expect(days[0]["deviceFamily"] as? String == "iphone")
...
@Test func analyticsTracksBoundedCumulativeErrors() async throws {
    ...
    try await client.trackError("model_load_failed", component: "generation", count: 2)
